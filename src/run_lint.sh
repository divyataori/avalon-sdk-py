...
lint_module() {
    ...
    if [ "$python_packages" != "" ]; then
        ...
        if [ "$python_packages" != "" ] ; then
            file $python_packages | grep CRLF && error=1
        fi
...
do
    case $MODULE in
        avalon_sdk_direct)
                lint_module avalon_sdk_direct                   "$SINCE" 1  0 || rv=1
                ;;
        common)
                lint_module common                              "$SINCE" 1  0 || rv=1
                ;;
        internal)
                lint_module internal                            "$SINCE" 1  0 || rv=1
                ;;
        avalon_sdk_proxy)
                lint_module avalon_sdk_proxy                    "$SINCE" 1  0 || rv=1
                ;;
        *)
